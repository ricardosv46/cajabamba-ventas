...
const Abono = () => {
	...
	const onSubmit = () => {
		createVentaEvento({
			input1: {
				...
				precioTotal: total,
				fechaVenta: fecha,
				razonSocial: values.tipoComprobante === 'Factura' ? values.razonSocial : values.nombres + '' + values.apellidos,
	...
	return (
		<>
			<PlantillaPage title='Evento' desc='Desde aqui podras vender los eventos' goback={onCloseForm}>
				<Show condition={!isOpenForm} className='pt-5'>
					<h2 className='text-xl text-center text-primary-500 dark:text-second-500'>SELECCIONA TUS ASIENTOS</h2>
				...
				<Show condition={isOpenForm}>
					<div className='flex flex-col items-center justify-center gap-10 mt-10 lg:flex-row '>
						<form onSubmit={form.handleSubmit} className='flex flex-col w-full gap-5 md:grid sm:grid-cols-2'>
							<Select
						...
						<div className='p-5 text-white rounded-lg bg-primary-100'>
							...
							{seleccionados.map((item) => {
								const desabilitado = asientos.some((desabilitado) => desabilitado?.reservado === item?.reservado)

								const newtiems = seleccionados.filter((seleccionado) => seleccionado.reservado !== item.reservado)
